...
public protocol NavigatorType {
    ...
    func moveRootViewController(_ completion: ((UIViewController?) -> Void)?)

    /// Move to targetClass controller
    func move<T>(_ target: T, completion: ((UIViewController?) -> Void)?) where T: UIViewController

    /// Move to targetInstance controller
    func move<T>(_ target: T.Type, completion: ((UIViewController?) -> Void)?) where T: UIViewController
}
...
extension NavigatorType {
    ...
    public func dismiss(_ animated: Bool, completion: (() -> Void)?) {
        guard let fromViewController = UIViewController.topMost?.navigationController else { return }
        fromViewController.dismiss(animated: animated, completion: nil)
    }
    ...
    public func moveRootViewController(_ completion: ((UIViewController?) -> Void)? = nil) {
        guard let rootViewController = UIApplication.shared.keyWindow?.rootViewController else {return}
        self.move(rootViewController, completion: completion)
    }

    public func move<T>(_ target: T, completion: ((UIViewController?) -> Void)? = nil) where T: UIViewController {
        self.moveProxy(nil, target, completion: completion)
    }

    public func move<T>(_ target: T.Type, completion: ((UIViewController?) -> Void)? = nil) where T: UIViewController {
        self.moveProxy(target, nil, completion: completion)
    }
...
// MARK: -  Move to targetType or targetInstance

extension NavigatorType {

    private func moveProxy<T>(_ targetType: T.Type? = nil,
                              _ targetInstance: T? = nil,
                              completion: ((UIViewController?) -> Void)? = nil) where T: UIViewController {
        let topVC = UIViewController.topMost
        ...
        topVC?.dismiss(animated: false, completion: {
            self.moveProxy(targetType, targetInstance, completion: completion)
        })
    }
}
