...
open class FormDirector: NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
        ...
        itemSizeCalculator: ItemSizeCalculator?)
    {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(didReceiveAction), name: NSNotification.Name(rawValue: BeeFormNotifications.CellAction), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(didReceiveAction), name: NSNotification.Name(rawValue: BeeFormNotifications.ElementKindAction), object: nil)
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, targetIndexPathForMoveFromItemAt originalIndexPath: IndexPath, toProposedIndexPath proposedIndexPath: IndexPath) -> IndexPath {
        return invoke(action: .canMoveTo, cell: collectionView.cellForItem(at: originalIndexPath), indexPath: originalIndexPath, userInfo: [BeeFormUserInfoKeys.CellCanMoveProposedIndexPath: proposedIndexPath]) as? IndexPath ?? proposedIndexPath
    }

    open func collectionView(_ collectionView: UICollectionView, moveItemAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        invoke(action: .move, cell: collectionView.cellForItem(at: sourceIndexPath), indexPath: sourceIndexPath, userInfo: [BeeFormUserInfoKeys.CellMoveDestinationIndexPath: destinationIndexPath])
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        return invoke(action: .showContextMenu, cell: collectionView.cellForItem(at: indexPath), indexPath: indexPath, userInfo: [BeeFormUserInfoKeys.ContextMenuInvokePoint: point]) as? UIContextMenuConfiguration
    }
