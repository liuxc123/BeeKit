...
public extension UIImage{
    ...
    static func bee_bundle(_ name:String, dark:String? = nil, forClass:AnyClass, from:String = "") -> UIImage {
        let from = from.isEmpty ? String(describing: forClass) : from
        guard #available(iOS 13.0, *), let dark = dark else {
            return UIImage(named: name, in: Bundle.bee_bundle(forClass, from), compatibleWith: nil) ?? UIImage()
        }
        switch UITraitCollection.current.userInterfaceStyle {
        case .dark:
            return UIImage(named: dark, in: Bundle.bee_bundle(forClass, from), compatibleWith: nil) ?? UIImage(named: name, in: Bundle.bee_bundle(forClass, from), compatibleWith: nil) ?? UIImage()
        default:
            return UIImage(named: name, in: Bundle.bee_bundle(forClass, from), compatibleWith: nil) ?? UIImage()
        }
    }
    
    static func bee_bundle(_ name:String, forClass:AnyClass, from:String = "") -> UIImage? {
        let from = from.isEmpty ? String(describing: forClass) : from
        return UIImage(named: name, in: Bundle.bee_bundle(forClass, from), compatibleWith: nil)
    }
