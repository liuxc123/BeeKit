//
//  NSRegularExpression+Bee.swift
//  BeeKit
//
//  Created by liuxc on 2021/1/5.
//

import Foundation

public extension Bee where Base: NSTextCheckingResult {
    /// 返回匹配字符串
    @discardableResult
...
public extension NSRegularExpression {
    static func bee_init(_ pattern: String, options: NSRegularExpression.Options = []) -> NSRegularExpression? {
        return Bee.makeRegEx(pattern, options:options)
    }
}

public extension Bee where Base: NSRegularExpression {
    ...
    func match(replaceIn string:String, with template:String, count: Int? = nil, options: NSRegularExpression.MatchingOptions = []) -> String {
        ...
        for item in ranged.reversed() {
            let replacement = item.bee.replace(in: string, template: template)
            if let range = item.bee.range(in: string) {
...
public extension Bee where Base == String {
    func regex(_ pattern:String) -> Bool {
        return Bee<NSRegularExpression>.makeRegEx(pattern)?.bee.match(base) ?? false
    }
}
