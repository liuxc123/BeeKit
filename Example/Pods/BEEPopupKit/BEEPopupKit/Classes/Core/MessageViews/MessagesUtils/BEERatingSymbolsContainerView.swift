//
//  BEERatingSymbolsContainerView.swift
//  BEEPopupView
//
...
final public class BEERatingSymbolsContainerView: UIView {

    private var message: BEERatingMessage!
    private var symbolsArray: [BEERatingSymbolView] = []

    public func setup(with message: BEERatingMessage,
                      externalSelection: @escaping BEERatingMessage.Selection) {
        ...
        for (index, item) in message.ratingItems.enumerated() {
            let itemView = BEERatingSymbolView(unselectedImage: item.unselectedImage,
