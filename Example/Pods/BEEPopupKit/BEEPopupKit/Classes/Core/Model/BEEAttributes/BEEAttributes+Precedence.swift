//
//  BEEAttributes+Precedence.swift
//  BEEPopupKit
//
...
fileprivate extension Int {
    var isValidDisplayPriority: Bool {
        return self >= BEEAttributes.Precedence.Priority.minRawValue && self <= BEEAttributes.Precedence.Priority.maxRawValue
    }
}
    ...
    enum Precedence {
        ...
        public enum QueueingHeuristic {
            ...
            var heuristic: EntryCachingHeuristic {
                switch self {
                case .chronological:
                    return BEEEntryChronologicalQueue()
                case .priority:
                    return BEEEntryPriorityQueue()
                }
            }
        }
...
public extension BEEAttributes.Precedence.Priority {
    ...
    static let max = BEEAttributes.Precedence.Priority(rawValue: maxRawValue)
    static let high = BEEAttributes.Precedence.Priority(rawValue: highRawValue)
    static let normal = BEEAttributes.Precedence.Priority(rawValue: normalRawValue)
    static let low = BEEAttributes.Precedence.Priority(rawValue: lowRawValue)
    static let min = BEEAttributes.Precedence.Priority(rawValue: minRawValue)
}
