...
class BEEContentView: UIView {
    ...
    // Dismissal handler
    var dismissHandler: BEEPopupKit.DismissCompletionHandler?

    // Data source
    private var attributes: BEEAttributes {
        return contentView.attributes
    }

    // Content
    private var contentView: BEEEntryView!
    ...
    func setup(with contentView: BEEEntryView) {
    ...
    private func setupOutConstraint(animation: BEEAttributes.Animation?, messageInAnchor: QLAttribute, priority: QLPriority) -> NSLayoutConstraint {
    ...
    private func setupOutConstraints(messageInAnchor: QLAttribute) {
        ...
        var popAnimation: BEEAttributes.Animation?
    ...
    // Setup layout constraints according to BEEAttributes.PositionConstraints
    ...
    // Animate out
    private func animateOut(with animation: BEEAttributes.Animation, outTranslationType: OutTranslation) {
    ...
    private func performAnimation(out: Bool, with animation: BEEAnimation, preAction: @escaping () -> () = {}, action: @escaping () -> ()) {
...
extension BEEContentView {
    ...
    private func animateRubberBandPullback() {
        ...
        let animation: BEEAttributes.Scroll.PullbackAnimation
